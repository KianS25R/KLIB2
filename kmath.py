...
def nrt(base: int|float, n: int|float):
    return base**(1/n)

def asin(number: int|float):
    return number+((number**3)/6)+((3*number**5)/40)+((5*number**7)/112)+((35*number**9)/1152)+((63*number**11)/2816)+((231*number**13)/13312)

def acos(number: int|float):
    return (pi()/2)-asin(number)


def acosdeg(number: int|float):
    return deg(acos(number))


def asindeg(number: int | float):
    return deg(asin(number))

def deg(radians: float|int):
    return radians*(180/pi())

def atan(number: int|float):
    if abs(number) <= 1:
...
def atan2(y: int|float, x: int|float):
    if x > 0:
        return atan(y/x)
    elif x < 0 and y >=0:
        return atan(y/x)+pi()
